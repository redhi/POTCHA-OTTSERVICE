...
def top_rated():
    ...
    response_movie = requests.get(
        "https://api.themoviedb.org/3/movie/popular?api_key=71ee3d022e284d4675ce65473e39a1b9&language=ko-KR&append_to_response=images&include_image_language=en,null"
    )
    response_tv = requests.get(
        "https://api.themoviedb.org/3/tv/popular?api_key=71ee3d022e284d4675ce65473e39a1b9&language=ko-KR&append_to_response=images&include_image_language=en,null"
    )
...
def movie_hit():
    ...
    return jsonify(
        [movies1_list, movies2_list, movies3_list, movies4_list, movies5_list]
    )
...
def search():
    ...
    movie_list_name = engine.execute(
        "SELECT * FROM Movie WHERE title LIKE %s ORDER BY (CASE WHEN SUBSTRING(title,1,1) RLIKE '[ㄱ-ㅎ가-힣]' THEN 1 WHEN SUBSTRING(title,1,1) RLIKE '[a-zA-Z]' THEN 2 ELSE 3 END),title",
        [search],
    )
    movie_list = [Movie.to_dict(movie) for movie in movie_list_name]

    tv_list_name = engine.execute(
        "SELECT * FROM Tv WHERE name LIKE %s ORDER BY (CASE WHEN SUBSTRING(name,1,1) RLIKE '[ㄱ-ㅎ가-힣]' THEN 1 WHEN SUBSTRING(name,1,1) RLIKE '[a-zA-Z]' THEN 2 ELSE 3 END),name",
        [search],
    )
