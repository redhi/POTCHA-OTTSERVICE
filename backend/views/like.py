...
def like():
    ...
    if like == True:  # 리스트에 있으면 추가 불가 중복 코드 작성
        if category == "movie":
            is_potato = Potato_Basket.query.filter(
                Potato_Basket.user_id == user_id, Potato_Basket.movie_id == content_id
            ).first()
        ...
        if category == "tv":
            is_potato = Potato_Basket.query.filter(
                Potato_Basket.user_id == user_id, Potato_Basket.tv_id == content_id
            ).first()
            ...
            Tv.query.filter_by(id=content_id).update({"like_count": tv.like_count})

        db.session.add(potato)

    if like == False:  # 리스트에 없으면 삭제 불가 코드 작성
        if category == "movie":
            potato = Potato_Basket.query.filter(
                Potato_Basket.user_id == user_id, Potato_Basket.movie_id == content_id
            ).first()
            movie = Movie.query.filter(Movie.id == content_id).first()
