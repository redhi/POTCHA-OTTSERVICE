...
def movie_list_sorted():
    ...
    if creiteria == "제목 순":
        movie_list_name = engine.execute(
            "SELECT * FROM Movie ORDER BY (CASE WHEN SUBSTRING(title,1,1) RLIKE '[ㄱ-ㅎ가-힣]' THEN 1 WHEN SUBSTRING(title,1,1) RLIKE '[a-zA-Z]' THEN 2 ELSE 3 END),title"
        )
...
def movie_list_filter():
    ...
    movie_list = [
        [Movie.to_dict(movie).get("genres"), Movie.to_dict(movie).get("id")]
