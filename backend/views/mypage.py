    ...
    if request.method == "POST":
        ...
        if user_id != user.id:
            is_friend = Friend.query.filter(
                Friend.user_id == user_id, Friend.nickname == nickname
            ).first()

            if not is_friend:
                friend = Friend(
                    user_id=user_id, nickname=user.nickname, photolink=user.photolink
                )
...
def potato_basket_recommend():
    ...
    with open("score_board.csv", "a", encoding="UTF-8", newline="") as output_file:
        ...
        for i in range(len(gen_score)):
            total_score = gen_score[i] * 0.5 + coun_score[i] * 0.3 + key_score[i] * 0.2
            f.writerow(
                [
                    contents_id[i],
                    gen_score[i],
