...
def tv_list_sorted():
    ...
    if creiteria == "제목 순":
        tv_list_name = engine.execute(
            "SELECT * FROM Tv ORDER BY (CASE WHEN SUBSTRING(name,1,1) RLIKE '[ㄱ-ㅎ가-힣]' THEN 1 WHEN SUBSTRING(name,1,1) RLIKE '[a-zA-Z]' THEN 2 ELSE 3 END),name"
        )
...
def tv_list_filter():
    ...
    tv_list = [
        [Tv.to_dict(tv).get("genres"), Tv.to_dict(tv).get("id")] for tv in tv_all
    ]
